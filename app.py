...
st.markdown("Enter any valid stock ticker below to pull live market data, view technical indicators, and train a custom LSTM model on-the-fly.")
...
lookback = st.sidebar.slider("Lookback Window (Days)", min_value=30, max_value=90, value=60)
...
if df is None or len(df) <= (lookback + 20):
    st.error(f"Could not retrieve enough data for '{ticker}'. Please check if the ticker symbol is correct or choose an earlier start date.")
    st.stop()

st.success(f"Successfully loaded {len(df)} trading records for **{ticker}**!")

with st.expander("View Raw Data Sample"):
    st.dataframe(df.tail(10), use_container_width=True)

st.subheader(f"Price Charts for {ticker}")
tab1, tab2 = st.tabs(["Candlestick Chart", "Moving Averages (MA100 & MA200)"])
...
with tab2:
    fig_ma = go.Figure()
    fig_ma.add_trace(go.Scatter(x=df["Date"], y=df["Close"], mode="lines", name="Close Price", line=dict(color="#38BDF8", width=1.5)))
    fig_ma.add_trace(go.Scatter(x=df["Date"], y=df["MA100"], mode="lines", name="100-Day MA", line=dict(color="#F59E0B", width=1.5)))
    fig_ma.add_trace(go.Scatter(x=df["Date"], y=df["MA200"], mode="lines", name="200-Day MA", line=dict(color="#10B981", width=1.5)))
...
if st.button(f"Run Prediction Model for {ticker}", type="primary"):
    ...
    with st.spinner(f"Training custom neural network for {ticker}... Please wait."):
        model.fit(
            xtrn,
            ytrn,
    ...
    fig_eval = go.Figure()
    fig_eval.add_trace(go.Scatter(y=ytst_actual.flatten(), mode="lines", name="Actual Price", line=dict(color="#94A3B8", width=1.5)))
    fig_eval.add_trace(go.Scatter(y=y_pred.flatten(), mode="lines", name="Predicted Price", line=dict(color="#EF4444", width=1.5, dash="dash")))
